/* ==========================================
   1. إعدادات قاعدة البيانات (الأجهزة والمهام)
   ========================================== */
const MINERS_DB = {
...
// 🟢 هنا يمكنك إضافة أو تعديل المهام بسهولة!
const TASKS_DB = [
...
let player = {
    ...
    lastDailyBonus: 0,
    miners: [0],
    completedTasks: [] // لحفظ المهام التي أكملها
};
...
if (savedData) {
    ...
    if(!player.completedTasks) player.completedTasks = []; // تأمين للمستخدمين القدامى
}
...
/* ==========================================
   5. التفاعلات (المهام، التنقل، الشراء)
   ========================================== */

// 🟢 التبديل بين الصفحات (التنقل السفلي)
...
function startTask(taskId, link) {
    // فتح الرابط (قناة التليجرام مثلاً)
    ...
    btn.innerText = "تحقق...";
    btn.style.backgroundColor = "#facc15"; // أصفر
    
    // محاكاة التحقق بعد 3 ثوانٍ وإعطاء المكافأة
    setTimeout(() => {
        ...
        gameLoop();
        renderTasks(); // إعادة رسم المهام لتصبح "مكتمل"
